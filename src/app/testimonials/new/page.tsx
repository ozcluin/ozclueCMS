...
export default function NewTestimonialPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setError(typeof data.error === 'string' ? data.error : 'Failed to add testimonial');
    }
    setSaving(false);
  }
            ...
            <div className="form-card">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                <div className="form-group">
                  ...
                  <input className="form-input" name="name" value={form.name} onChange={handleChange} required placeholder="e.g. Jane Doe" />
                </div>
                <div className="form-group">
                  <label className="form-label">Company / Client Organization</label>
                  <input className="form-input" name="company" value={form.company} onChange={handleChange} placeholder="e.g. Acme Corp" />
                </div>
                <div className="form-group">
                  <label className="form-label">Role / Job Title</label>
                  <input className="form-input" name="role" value={form.role} onChange={handleChange} placeholder="e.g. HR Manager" />
                </div>
                <div className="form-group">
